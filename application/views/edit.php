							...
							<div class="row">
								<div class="col-md-12">
									<div class="form-group">
										...
										<input type="text" name="nama" id="nama" value="<?= set_value('nama')?set_value('nama'):$product['nama_produk'] ?>" class="form-control">
										<?= form_error('nama', '<small class="text-danger pl-3" id="error-nama">', '</small>'); ?>
									</div>
									<div class="form-group">
										<label for="harga">Harga</label>
										<input type="number" name="harga" id="harga" class="form-control" value="<?= set_value('harga')?set_value('harga'):$product['harga'] ?>">
										<?= form_error('harga', '<small class="text-danger pl-3" id="error-harga">', '</small>'); ?>
									</div>
									<div class="form-group">
										<label for="stok">Stok</label>
										<input type="text" name="stok" id="stok" class="form-control" value="<?= set_value('stok')?set_value('stok'):$product['jumlah'] ?>">
										<?= form_error('stok', '<small class="text-danger pl-3" id="error-stok">', '</small>'); ?>
									</div>
									<div class="form-group">
										<label for="keterangan">Keterangan</label>
										<textarea name="keterangan" id="keterangan" class="form-control" cols="30" rows="4" style="resize: none;"><?= set_value('keterangan')?set_value('keterangan'):$product['keterangan'] ?></textarea>
